...
List topMenu(0, 1, 18, 4, 3, (char**)&_main_menu);
List subMenu0(0, 1, 18, 5, 3, (char**)&_sub_menu_0);
...
void Td5Menu::drawMenu()
{
  ...
  if(!hidden)
  {
    ...
    lcd.clear();
    lcd.setCursor(0,0);
    strlcpy_P(list_buffer,(char*)pgm_read_word(menu_header+current_header), 21);
...
//Interactions
byte Td5Menu::wait_on_escape(int ref_time) // Returns key pressed or 0 if time expires before any key was pressed.
{
  //Wait on button push.
  long temp0;
  byte temp1;
