...
byte Td5Remote::recvCommand()
{
  ...
  if(remoteSerial.available() > 0) 
  {
    while(read_byte(&dataCaught))
    {
      gotData = true;        
      #ifdef _DEBUG_
      Serial.print(P("Received: "));Serial.println(dataCaught,HEX);
      #endif
      if(responseIndex > REMOTE_MAX_COMMAND_LENGTH)  
      {
        #ifdef _DEBUG_
        Serial.print(P("Command too long"));
        #endif          
    ...
    {
      #ifdef _DEBUG_
      Serial.println(P("Wrong checksum"));
      #endif      
      aknCommand(false);
      return REMOTE_COMM_FAIL;
    }
    ...
    {
      #ifdef _DEBUG_
      Serial.print(P("Command: "));Serial.println(commandBuffer[1],HEX);
      #endif
    ...
    {
      #ifdef _DEBUG_
      Serial.println(P("Wrong telegram structure"));
      #endif
      aknCommand(false);      
      return REMOTE_COMM_FAIL;
    }
...
void Td5Remote::aknCommand(boolean positiveAnswer)
{
  ...
  #ifdef _DEBUG_
  Serial.println(P("Reset actual command"));
  #endif  
  command = 0x00; 
}
